...
from pbf.controller.PbfStruct import yamldata
...
class basic(PBF):
    ...
    def messageListener(self):
            ...
            if mlo == 404:
                ...
                if mlo.get('uid') == uid:
                    if mlo.get('times') >= int(settings._get('AntiswipeScreen')):
                        ...
                        messagelist[arrnum]['times'] += 1
                    # 禁言警告
                    if mlo.get('times') == int(settings._get('AntiswipeScreen'))-1 and se.get('sender').get('role') == "member":
                        client.msg(
        ...
        if message[0:10] == '[CQ:reply,' and '撤回' in message:
            if uid == botSettings._get('owner') or uid == botSettings._get('second_owner') or se.get('sender').get('role') != 'member':
    ...
    def noticeListener(self):
        ...
        if se.get('notice_type') == 'group_ban' and se.get('user_id') == se.get('self_id'):
        ...
        elif se.get('notice_type') == 'group_recall' and settings._get('recallFlag') != 0 and se.get('operator_id') != botSettings._get('myselfqn') and se.get('user_id') != botSettings._get('myselfqn'):
            # 消息防撤回
            ...
            # 戳机器人
            if se.get('sub_type') == 'poke' and se.get('target_id') == botSettings._get('myselfqn') and botSettings._get("chuo"):
    ...
    def checkBan(self):
        ...
        self.mysql.commonx("UPDATE `botSettings` SET `bannedCount`=%s WHERE `qn`=%s", (int(self.data.groupSettings._get("bannedCount"))+1, self.data.se.get("group_id")))
        ...
        if int(self.data.groupSettings._get("bannedCount"))+1 >= int(self.data.botSettings._get("bannedCount")):
            ...
            if self.data.groupSettings._get("connectQQ"):
                self.client.msg(TextStatement("[自动消息] 请注意，您关联的群组（{}）因违反机器人规定致使机器人退群，您将会承担连带责任".format(self.data.se.get("group_id")))).custom(self.data.groupSettings._get("connectQQ"))
            self.client.msg(TextStatement("[提示] 群：{}\n关联成员：{}\n行为：禁言{}次\n已自动退群".format(self.data.se.get("group_id"), self.data.groupSettings._get("connectQQ"), int(self.data.groupSettings._get("bannedCount"))+1))).custom(self.data.botSettings._get("owner"))
